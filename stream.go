...
func newClientStream(ctx context.Context, desc *StreamDesc, cc *ClientConn, method string, opts ...CallOption) (_ ClientStream, err error) {
	...
	if err := cc.waitForResolvedAddrs(ctx); err != nil {
		...
		if elapsed1 > 100*time.Millisecond {
			grpclog.Warningf("grpcdebug: newClientStream: target=%v waitForResolvedAddrs time: %v", cc.Target(), elapsed1)
		}
		return nil, err
	}
	elapsed1 := time.Since(now1)
...
func (cs *clientStream) withRetry(op func(a *csAttempt) error, onSuccess func()) error {
	...
	defer func() {
		...
		if elapsed > 300*time.Millisecond {
			grpclog.Warningf("grpcdebug: clientStream: target=%v withRetry all time: %v", cs.cc.Target(), elapsed)
		}
	}()
	...
	printer := func(msg string) {
		...
		if elapsed > 100*time.Millisecond {
			grpclog.Warningf("grpcdebug: clientStream: target=%v withRetry %s time: %v, retry %d p=%p", cs.cc.Target(), msg, elapsed, cs.numRetries, cs.attempt)
		}
	}
...
func (cs *clientStream) SendMsg(m interface{}) (err error) {
	...
	defer func() {
		elapsed := time.Since(now)
		if elapsed > 200*time.Millisecond {
			grpclog.Warningf("grpcdebug: clientStream: target=%v SendMsg time: %v", cs.cc.Target(), elapsed)
		}
...
func (cs *clientStream) RecvMsg(m interface{}) error {
	...
	defer func() {
		elapsed := time.Since(now)
		if elapsed > 300*time.Millisecond {
			grpclog.Warningf("grpcdebug: clientStream: target=%v RecvMsg time: %v", cs.cc.Target(), elapsed)
		}
	}()
